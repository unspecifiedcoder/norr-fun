 ...
 */ type LogoProps = {
  /** Any CSS length. Everything scales from this. */ size?: string; className?: string;
}; const WORDMARK = "NORR.FUN"; export const Logo = ({ size = "2.25rem", className = "" }: LogoProps) => (
  <span className={`norr-logo ${className}`} style={{ fontSize: size }} role="img" aria-label="norr.fun"
  >
    <span className="norr-logo__stack">
      <span className="norr-logo__ghost norr-logo__ghost--a" aria-hidden="true">
        {WORDMARK}
      </span>
      <span className="norr-logo__ghost norr-logo__ghost--b" aria-hidden="true">
        {WORDMARK}
      </span>
