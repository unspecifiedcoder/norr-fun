  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      for (const l of logs) {
        ...
        if (a.from !== ZERO) {
          balances.set(a.from, (balances.get(a.from) ?? 0n) - a.value);
        }
        if (a.to !== ZERO) {
          balances.set(a.to, (balances.get(a.to) ?? 0n) + a.value);
        }
      }
      ...
      {holders.length === 0 ? (
        ...
        <ul className="space-y-2">
          {holders.slice(0, 20).map((h, i) => (
            <li
              key={h.address}
              className="flex items-center gap-3 p-3 rounded-lg border border-gray-700 text-xs"
            >
              ...
              <span className="text-gray-500 w-14 text-right">{h.share.toFixed(2)}%</span>
            </li>
          ))}
        </ul>
      )}
