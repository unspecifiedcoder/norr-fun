...
export const ChainGuard = ({ children }: { children: React.ReactNode }) => {
  ...
  const wanted = chain ? CHAIN_SLUGS[chain.toLowerCase()] : undefined;

  useEffect(() => {
    if (chain && wanted === undefined) {
      // Unknown slug: nothing to guard against, fall through to the app.
    }
  }, [chain, wanted]);

  if (chain && wanted === undefined) {
    return <Navigate to={location.pathname.replace(`/${chain}`, "")} replace />;
  }

  if (wanted !== undefined && wanted !== current) {
    return (
      <div className="border border-dashed border-amber-800 rounded-xl p-10 text-center">
        <p className="text-amber-400 font-bold">This link is for another network</p>
        <p className="text-sm text-gray-400 mt-2 max-w-md mx-auto">
          It points at <span className="text-gray-200">{chain}</span>, but your
          wallet is on chain {current}. Switch networks to open it — nothing here
          will change your wallet for you.
        </p>
      </div>
    );
  }
