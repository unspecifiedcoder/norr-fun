  ...
  if (!b.available) {
    return (
      <Notice
        ...
        body={`No board registry is deployed on chain ${b.chainId}.`}
      />
    );
  }
    ...
    <>
      <Card title="Publisher desks">
        <p className="text-gray-400 text-sm mb-4">
          A desk is a surface you run. Raises published through it route a share
          of what they raise to you, and the terms are enforced on-chain when a
          raise registers — not left to whoever built the client.
        </p>
          ...
          <div className="mt-5 flex items-center gap-4 flex-wrap">
            <ActionButton
              onClick={async () => {
                ...
                if (ok) {
                  setForm({ slug: "", name: "", description: "", minPartnerPercent: "", open: true });
                  setOpen(false);
                }
              }}
            ...
            {b.status && <p className="text-xs text-gray-300">{b.status}</p>}
          </div>
        </Card>
      )}
...
const BoardRow = ({ board }: { board: Board }) => (
  ...
  >
    <span className="w-9 h-9 rounded-lg bg-gradient-to-br from-violet-500/25 to-cyan-500/25 border border-gray-600 grid place-items-center text-[10px] font-bold shrink-0 uppercase">
      {board.slug.slice(0, 2)}
    </span>
    <span className="min-w-0 flex-1">
      <span className="block text-sm font-bold text-gray-100 truncate">
        {board.name}
      </span>
    ...
    <span className="text-right shrink-0">
      ...
      <span className="block text-[10px] text-gray-600 flex items-center gap-1 justify-end">
...
export const BoardDetail = () => {
  ...
  return (
    <>
      ...
      >
        <FaArrowLeft className="text-[10px]" /> All desks
      </Link>
